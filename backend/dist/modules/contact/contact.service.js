...
function buildAdminEmail(data, submittedAt) {
    ...
    const chip = (text, bg = '#f0f7ee', color = '#4a703f', border = '#c6dcc2') => `<span style="display:inline-block;background:${bg};color:${color};border:1px solid ${border};
    ...
    const row = (label, value) => value.trim()
        ? `<tr>
        ...
        : '';
    const section = (title, body) => `<tr><td style="padding:20px 0 0;">
...
<table width="600" cellpadding="0" cellspacing="0"
  ...
  <table width="100%" cellpadding="0" cellspacing="0">
    ...
    ${section('Basic Details', `<table cellpadding="0" cellspacing="0" width="100%">
    ...
    <!-- Section 2: You Are A -->
    ${roles.length ? section('You Are A', `<div>${roles.map(r => chip(r)).join('')}</div>`) : ''}

    <!-- Section 3: Area of Interest -->
    ${interests.length ? section('Area of Interest', `<div>${interests.map(i => chip(i, '#fff7ed', '#c2410c', '#fed7aa')).join('')}</div>`) : ''}

    <!-- Section 4: Product Interest -->
    ${products.length ? section('Product Interest', `<div>${products.map(p => chip(p, '#f0fdf4', '#15803d', '#bbf7d0')).join('')}</div>`) : ''}

    <!-- Section 5: Message -->
    ${section('Message / Requirement', `<div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;
...
class ContactService {
    static async create(data) {
        ...
        const record = await db_1.prisma.contactMessage.create({
            data: {
                name: data.name,
                email: data.email,
                phone: data.phone,
        ...
        logger_1.logger.info(`[Contact] New submission from ${data.email} — admin target: ${adminEmail ?? 'NOT SET'}`);
        if (!adminEmail) {
            logger_1.logger.error('[Contact] ADMIN_EMAIL_TO is not set in .env — email skipped');
            return record;
        }
        try {
            ...
            logger_1.logger.info(`[Contact] Admin notification delivered to ${adminEmail}`);
        }
        catch (err) {
            logger_1.logger.error(`[Contact] Admin email FAILED: ${err?.message}`);
    ...
    static async list(page = 1, limit = 20, search) {
        ...
        const [messages, total] = await Promise.all([
            db_1.prisma.contactMessage.findMany({ where, orderBy: { createdAt: 'desc' }, skip, take: limit }),
